...
class ExcelProcessor:
    ...
    def format_iso_date(self, excel_date):
        try:
            ...
            self.logger.warning(f"Invalid date format encountered: {excel_date}")
            return None
    ...
    def write_records_to_file(self, output_file_path, records):
        try:
            ...
            self.logger.info(f"File written successfully: {output_file_path}")
            return len(records)
        except Exception as e:
            self.logger.error(f"Failed to write file {output_file_path}: {str(e)}")
            raise
    ...
    def process_excel_file(self):
        try:
            ...
            if not metadata_sheets:
                self.logger.error("No metadata sheets found in the Excel file.")
                return

            self.logger.info(f"Processing Excel file: {self.input_file}")
            for sheet_name in metadata_sheets:
                ...
                cost_center = (
                    str(sheet["B3"].value).zfill(12)
                    if isinstance(sheet["B3"].value, (int, float))
                    else sheet["B3"].value.strip()
                )
                ...
                for row in sheet.iter_rows(min_row=9, values_only=True):
                    ...
                    # Serialize current records to calculate size
                    serialized_data = "\n".join(json.dumps(rec, separators=(",", ":")) for rec in current_records).encode("utf-8")
                    current_file_size = len(serialized_data)

                    if current_file_size > size_limit:
                        output_file_path = os.path.join(
                            self.output_directory, f"{excel_base_name}_{sheet_name}_{file_counter}.a360"
                        )
                        self.write_records_to_file(output_file_path, current_records[:-2])  # Exclude last 2 records
                        self.logger.info(f"File split triggered at 10 MB for {output_file_path}")
                        current_records = current_records[-2:]  # Carry over last 2 records to the next file
            ...
            self.logger.info(f"Completed processing Excel file: {self.input_file}")
